...
def handmark_transform(samples):
    feats = np.ones((0,441))
    labels = np.array([])
    with mp_hands.Hands(
        ...
        for idx, (file, label) in enumerate(samples):                
            if "depth_" not in file:
                ...
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    feat_vec = np.expand_dims(hm.feature_vector, axis=0)
                    feats = np.append(feats, feat_vec, axis=0)
